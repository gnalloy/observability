...
装配说明：
- observability 模块供 handler、transport、应用或 adapter 使用，用于指标和 tracing 契约。
- 除非模块明确负责 adapter，否则 vendor-specific exporter 不应进入热路径核心包。
- metric 名称、cardinality 与 label 需要在压测下验证。
