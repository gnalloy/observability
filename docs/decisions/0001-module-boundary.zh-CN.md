# ADR-001：保持 observability 为独立 Gnalloy 模块
...
`observability` 作为 `gnalloy.org/observability` 发布，并拥有该职责：厂商无关的 Gnalloy observability 契约、低开销 recorder 与 Prometheus text export。
